...
const useModal = (handleHardBack = true): any => {
    const [isUseBackHandle, setIsUseBackHandle] = useState(false);
    let backHandler: any;
    useEffect(() => {
        if (handleHardBack) {
            backHandler = BackHandler.addEventListener('hardwareBackPress', () => {
                dismiss();
                return isUseBackHandle;
            });
        }
        return () => {
            handleHardBack && backHandler.remove();
        };
    }, [isUseBackHandle]);
